...
DEFAULT_IMAGE = default_image((150, 34))
...
class Textbox(object):
    def __init__(self, parent, position, font=None, color=None, image=DEFAULT_IMAGE, return_call=None):
    ...
    def on_keydown(self, event):
        if self.selected:
            if 32 <= event.key < 123:
                ...
                if self.return_call and len(self.hidden.text) > 0:
                    self.return_call(self.hidden.text)
                    self.hidden.text = ""
